...
class AutoViewManager {
    ...
    // Fired when someone in a group references a status. Marks the GSM
    // notification message itself as read in the group chat — does NOT touch
    // the underlying status broadcast (that belongs to the original poster).
    async viewGroupStatusMention(sock, msg) {
        try {
            if (!this.config.enabled || !this.config.settings.markAsSeen) return false;
            if (!msg?.key || msg.key.fromMe) return false;
            const sender = (msg.key.participant || msg.key.remoteJid || '').split('@')[0].split(':')[0];
            const displayId = '+' + sender;
            if (this.isExcluded(msg.key)) {
                logBox(displayId, '👥 GSM', `${Y}SKIPPED — excluded${X}`);
                return false;
            }
