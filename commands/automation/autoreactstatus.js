...
// ── Group-Status-Mention (GSM) view ────────────────────────────────────────
// When someone references a status inside a group, mark the GSM notification
// itself as read in the group chat — even if `enabled` is on. We only VIEW,
// we never send a status reaction (the GSM is a regular group message, not a
// real status@broadcast item).
export async function handleAutoReactGsmView(sock, msg) {
    try {
        if (!autoReactManager.config.enabled) return false;
        if (!msg?.key || msg.key.fromMe) return false;
        if (autoReactManager.isExcluded(msg.key)) return false;
        await sock.readMessages([{
        ...
        console.log(`\x1b[32m✅ [AUTOREACT] GSM viewed from +${sender}\x1b[0m`);
        return true;
    } catch (_) {
        return false;
    }
