...
export function setConfigBotId(botId) {
    if (botId) {
        ...
        // Mirror to MongoDB async (fire-and-forget)
        if (configBotId !== 'default' && globalThis.mongo?.isReady) {
            globalThis.mongo.upsertConfig('default', '__bot_phone__', configBotId).catch(() => {});
        }
    }
}
...
export async function setConfig(key, value) {
    ...
    // Mirror to MongoDB async (fire-and-forget)
    if (globalThis.mongo?.isReady) {
        globalThis.mongo.upsertConfig(botId, key, jsonValue).catch(() => {});
    }
...
export async function setAutoConfig(key, value) {
    ...
    // Mirror to MongoDB async (fire-and-forget)
    if (globalThis.mongo?.isReady) {
        globalThis.mongo.upsertConfig(botId, `auto_config:${key}`, jsonValue).catch(() => {});
    }
