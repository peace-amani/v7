...
// ── MongoDB auth state factory ─────────────────────────────────────────────
// Stores WhatsApp session credentials and Signal keys in MongoDB collections.
// Drop-in replacement for useSQLiteAuthState when a MongoDB connection is available.
//
// Usage:
//   const { state, saveCreds } = await useMongoAuthState(mongoAdapter);
//
// The mongoAdapter argument must be the default export of lib/mongoAdapter.js.
// If mongo is not ready (MONGODB_URI not set or connection failed) this function
// falls back to the file-based session store automatically.
export async function useMongoAuthState(mongoAdapter, sessionDir = './session') {
    await getBaileysDeps();

    // If mongo is not ready, fall back to file-based storage gracefully
    if (!mongoAdapter?.isReady) {
    ...
    const BufferJSON    = _BufferJSON;
    const initAuthCreds = _initAuthCreds;
    const { SessionCred, SessionKey } = mongoAdapter.models;

    // ── Helpers ────────────────────────────────────────────────────────────
    async function readKey(type, id) {
        try {
            const doc = await SessionKey.findOne({ type, id }).lean();
            if (!doc) return null;
    ...
    async function deleteKey(type, id) {
        try { await SessionKey.deleteOne({ type, id }); } catch {}
    }

    async function readCreds() {
        try {
            const doc = await SessionCred.findOne({ key: 'creds' }).lean();
            if (!doc) return null;
            return JSON.parse(doc.value, BufferJSON.reviver);
        } catch { return null; }
    }
    ...
    async function writeCreds(creds) {
        try {
            const json = JSON.stringify(creds, BufferJSON.replacer);
            await SessionCred.findOneAndUpdate(
                { key: 'creds' },
                { $set: { value: json, updated_at: new Date() } },
                { upsert: true }
            );
        } catch (err) {
            console.error('⚠️ MongoAuthState: Failed to save creds:', err.message);
        }
    }
            ...
            keys: {
                ...
                set: async (data) => {
                    ...
                    for (const type in data) {
                        for (const id in data[type]) {
                            const value = data[type][id];
                            if (value) writes.push(writeKey(type, id, value));
                            else        deletes.push(deleteKey(type, id));
                        }
                    }
                    await Promise.allSettled([...writes, ...deletes]);
                }
            }
        },
        saveCreds: () => { writeCreds(creds); }
